...
function createEmptyLifelinesUsed() {
  return {
    1: { hint: false, mcq: false, doubleAnswer: false },
    2: { hint: false, mcq: false, doubleAnswer: false },
    3: { hint: false, mcq: false, doubleAnswer: false },
  };
}
...
function normalizeLifelinesUsed(raw) {
  ...
  for (const team of SUPPORTED_TEAM_COUNTS) {
    normalized[team] = {
      hint: !!raw?.[team]?.hint,
      mcq: !!raw?.[team]?.mcq,
      doubleAnswer: !!raw?.[team]?.doubleAnswer,
    };
  }
  return normalized;
}
...
const CATEGORY_DISPLAY_GROUPS = [
  {
    name: "علوم إسلامية",
    categories: ["القرآن الكريم", "أحاديث", "السيرة النبوية", "تاريخ إسلامي", "تاريخ اسلامي", "قصص الأنبياء"],
  },
  {
    name: "جغرافيا",
...
const CATEGORY_HELP_TEXTS = {
  ...
  "خمن اللغة من الكلمة": "سترى كلمة من لغة معينة وعليك تحديد اللغة",
  ...
  "اسم الكسر من الأشعة": "تُعرض صورة أشعة طبية، وعليك تحديد نوع الكسر الصحيح.",
};
...
function cacheElements() {
  ...
  doubleAnswerLifelineBtn: document.getElementById("doubleAnswerLifelineBtn"),
  choicesBox: document.getElementById("choicesBox"),
  choicesList: document.getElementById("choicesList"),
  hintBox: document.getElementById("hintBox"),
  hintText: document.getElementById("hintText"),
  doubleAnswerStatus: document.getElementById("doubleAnswerStatus"),
  ...
  teamShuffleModal: document.getElementById("teamShuffleModal"),
  teamShuffleNamesInput: document.getElementById("teamShuffleNamesInput"),
  teamShuffleGenerateBtn: document.getElementById("teamShuffleGenerateBtn"),
  teamShuffleReshuffleBtn: document.getElementById("teamShuffleReshuffleBtn"),
  teamShuffleResult: document.getElementById("teamShuffleResult"),
  teamShuffleContinueBtn: document.getElementById("teamShuffleContinueBtn"),
  teamShuffleSkipBtn: document.getElementById("teamShuffleSkipBtn"),
...
function ensureHomepageFlowModals() {
  const requiredModals = [
    ...
    },
    {
      id: "teamShuffleModal",
      html: `
        <div id="teamShuffleModal" class="modal hidden" role="dialog" aria-modal="true" aria-labelledby="teamShuffleTitle">
          <div class="modal-content team-shuffle-modal-content">
            <h2 id="teamShuffleTitle">توزيع اللاعبين على الفرق (اختياري)</h2>
            <p class="team-shuffle-intro">اكتب أسماء اللاعبين كل اسم في سطر مستقل، ثم وزّعهم عشوائيًا على الفرق بشكل متوازن.</p>
            <label for="teamShuffleNamesInput" class="team-name-label">أسماء اللاعبين</label>
            <textarea id="teamShuffleNamesInput" class="team-shuffle-input" rows="6" placeholder="أحمد&#10;محمد&#10;سارة"></textarea>
            <div class="modal-actions">
              <button id="teamShuffleGenerateBtn" class="secondary-btn" type="button">توزيع عشوائي</button>
              <button id="teamShuffleReshuffleBtn" class="secondary-btn hidden" type="button">إعادة التوزيع</button>
            </div>
...
const state = {
  ...
  doubleAnswerLifelineActive: false,
...
function resetDoubleAnswerLifelineState() {
  state.doubleAnswerLifelineActive = false;
  state.doubleAnswerAttemptsRemaining = 0;
  if (el.doubleAnswerStatus) {
    el.doubleAnswerStatus.textContent = "";
    el.doubleAnswerStatus.classList.add("hidden");
  }
}

function updateDoubleAnswerStatus() {
  if (!el.doubleAnswerStatus) return;
  if (!state.doubleAnswerLifelineActive) {
    el.doubleAnswerStatus.textContent = "";
    el.doubleAnswerStatus.classList.add("hidden");
    return;
  }
  ...
  el.doubleAnswerStatus.classList.remove("hidden");
}
...
function updateQuestionActionLock() {
  ...
  el.doubleAnswerLifelineBtn.disabled = disableAnsweringActions || state.answerRevealed || hasUsedLifeline(state.currentTeam, "doubleAnswer") || lockByVideo;
...
function persistLocalProgress() {
  ...
  const payload = {
    ...
    doubleAnswerLifelineActive: !!state.doubleAnswerLifelineActive,
    doubleAnswerAttemptsRemaining: Number(state.doubleAnswerAttemptsRemaining) || 0,
...
function applySavedLocalProgress(saved) {
  try {
    ...
    lifelinesUsed = normalizeLifelinesUsed(saved.lifelinesUsed || {
      1: { hint: !!saved.hintHelpUsed?.[1], mcq: !!saved.mcqHelpUsed?.[1], doubleAnswer: false },
      2: { hint: !!saved.hintHelpUsed?.[2], mcq: !!saved.mcqHelpUsed?.[2], doubleAnswer: false },
      3: { hint: !!saved.hintHelpUsed?.[3], mcq: !!saved.mcqHelpUsed?.[3], doubleAnswer: false },
    });
    state.doubleAnswerLifelineActive = !!saved.doubleAnswerLifelineActive;
    state.doubleAnswerAttemptsRemaining = Math.max(0, Number(saved.doubleAnswerAttemptsRemaining) || 0);
    ...
    if (saved.modalOpen && savedActiveId) {
      ...
      openQuestion(savedActiveId, {
        ...
        doubleAnswerLifelineActive: !!saved.doubleAnswerLifelineActive,
        doubleAnswerAttemptsRemaining: Math.max(0, Number(saved.doubleAnswerAttemptsRemaining) || 0),
      });
    }
...
function renderQuestionContent(question, { resetLifecycleState = true } = {}) {
  ...
  if (resetLifecycleState) {
    ...
    resetDoubleAnswerLifelineState();
  ...
  el.doubleAnswerLifelineBtn.disabled = hasUsedLifeline(state.currentTeam, "doubleAnswer") || (online.mode === "online" && !canCurrentClientAct());
...
async function openQuestion(tileId, {
  restored = false,
  deadlineTs = null,
  questionId = "",
  skipTurnGuard = false,
...
async function processPendingLifelineRequest(room, remoteGameState) {
  ...
  try {
    ...
    if (selectedRequest.type === "hint") {
      await useHintLifeline({ skipHostGuard: true, skipTurnGuard: true });
      return;
    }
    await useDoubleAnswerLifeline({ skipHostGuard: true, skipTurnGuard: true });
  } finally {
...
async function useDoubleAnswerLifeline({ skipHostGuard = false, skipTurnGuard = false } = {}) {
  const question = getActiveQuestion(); const currentTeam = state.currentTeam;
...
function applyScore(isCorrect, { skipHostGuard = false, skipTurnGuard = false } = {}) {
  ...
  if (!isCorrect) {
    if (state.doubleAnswerLifelineActive && state.doubleAnswerAttemptsRemaining > 1) {
      state.doubleAnswerAttemptsRemaining -= 1;
      updateDoubleAnswerStatus();
      if (online.mode === "online" && !online.applyingRemote) pushOnlineState();
      return;
    }
...
async function applyRemoteGameState(game, { applyNonce = 0 } = {}) {
  ...
  try {
    ...
    lifelinesUsed = normalizeLifelinesUsed(game.lifelinesUsed || {
      1: { hint: !!game.hintHelpUsed?.[1], mcq: !!game.mcqHelpUsed?.[1], doubleAnswer: false },
      2: { hint: !!game.hintHelpUsed?.[2], mcq: !!game.mcqHelpUsed?.[2], doubleAnswer: false },
      3: { hint: !!game.hintHelpUsed?.[3], mcq: !!game.mcqHelpUsed?.[3], doubleAnswer: false },
    });
    state.doubleAnswerLifelineActive = !!game.doubleAnswerLifelineActive;
    state.doubleAnswerAttemptsRemaining = Math.max(0, Number(game.doubleAnswerAttemptsRemaining) || 0);
      ...
      if (tile) {
        try {
          ...
          if (remoteQuestion) {
            ...
            el.doubleAnswerLifelineBtn.disabled = remoteTimedOut || hasUsedLifeline(state.currentTeam, "doubleAnswer") || !canCurrentClientAct();
            updateDoubleAnswerStatus();
            updateQuestionActionLock();
            el.modal.classList.remove("hidden");
            el.modal.classList.add("is-open");
...
function normalizePlayerNamesInput(rawValue = "") {
...
function buildTeamAssignments(players = [], teamCount = 2) {
  const normalizedTeamCount = normalizeTeamCount(teamCount);
  const preparedPlayers = players.map((name, index) => {
    const normalized = normalizeCell(name);
    return normalized ? { key: `${normalized}-${index}`, name: normalized } : null;
  }).filter(Boolean);
  const shuffledPlayers = shuffle(preparedPlayers);
  const assignments = {};
  for (let team = 1; team <= normalizedTeamCount; team += 1) {
    assignments[team] = [];
  }
  shuffledPlayers.forEach((player, index) => {
    const targetTeam = (index % normalizedTeamCount) + 1;
    assignments[targetTeam].push(player.name);
  });
  return assignments;
}

function renderTeamShuffleAssignments(assignments) {
  if (!el.teamShuffleResult) return;
  const normalizedAssignments = assignments && typeof assignments === "object" ? assignments : null;
  if (!normalizedAssignments) {
    el.teamShuffleResult.innerHTML = "";
    el.teamShuffleResult.classList.add("hidden");
    if (el.teamShuffleReshuffleBtn) el.teamShuffleReshuffleBtn.classList.add("hidden");
    return;
  }
  el.teamShuffleResult.innerHTML = "";
  getActiveTeamNumbers().forEach((team) => {
    const card = document.createElement("article");
    card.className = "team-shuffle-team";
    const names = Array.isArray(normalizedAssignments[team]) ? normalizedAssignments[team] : [];
    card.innerHTML = `<h3>${state.teamNames[team] || `الفريق ${team}`}</h3><p>${names.length ? names.join(" • ") : "لا يوجد لاعبين بعد"}</p>`;
    el.teamShuffleResult.appendChild(card);
  });
  el.teamShuffleResult.classList.remove("hidden");
  if (el.teamShuffleReshuffleBtn) el.teamShuffleReshuffleBtn.classList.remove("hidden");
}
...
function closeTeamShuffleModal() {
  if (!el.teamShuffleModal) return;
  el.teamShuffleModal.classList.remove("is-open");
  el.teamShuffleModal.classList.add("hidden");
}

function generateTeamShuffleAssignments() {
  clearError();
  const players = normalizePlayerNamesInput(el.teamShuffleNamesInput?.value || "");
  if (!players.length) {
    teamShuffleSetupState.assignments = null;
    renderTeamShuffleAssignments(null);
    showError("أدخل اسم لاعب واحد على الأقل لتفعيل التوزيع العشوائي.");
    return;
  }
  teamShuffleSetupState.assignments = buildTeamAssignments(players, teamShuffleSetupState.teamCount);
  renderTeamShuffleAssignments(teamShuffleSetupState.assignments);
}
...
function initializeApp() {
  ...
  bindEvent(el.doubleAnswerLifelineBtn, "click", useDoubleAnswerLifeline, "doubleAnswerLifelineBtn");
